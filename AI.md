...
### A-Assertions
- Updated parsing and list index validation so user-facing errors are handled via DukeException/guards rather than AssertionError.
...
- Fixed task index validation boundary bug (prevented IndexOutOfBoundsException and ensured consistent DukeException behavior).
- Removed unused/unsafe imports and reduced brittle dependencies.

### Javadoc
- Added missing JavaDoc for public methods across UI, parser, storage, serialization, and task classes to improve maintainability and meet style expectations.
